...
uncert = pd.read_excel("uncert_testing.xlsx", usecols = "H:X", skiprows = [1, 2, 3, 4])

additives = uncert.columns.tolist()
savedlows= lowAdditiveFractions
for i in additives:
    for b in range(50):
        lowAdditiveFractions[i]=uncert[i][b]
        select2018()
        assignValues()
        makeCalculations(True, [False])
        lowAdditiveFractions = {"Plasticizer":0.1, "Flame Retardant":0.007, "UV Stabilizer": 0.005, "Heat Stabilizer":0.005, "Antioxidant":0.005, "Slip Agent":0.001, "Lubricant":0.001, 
...
chunks = chunk_list(sensitivityPoints, 50)

data = dict(zip(additives, chunks))

new_df= pd.DataFrame(data)
        
new_df.to_excel("MonteCarloResults.xlsx")
